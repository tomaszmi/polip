...
class Value : public boost::spirit::extended_variant<
                  Null, bool, int64_t, double, std::string, Array, Object>
{
public:
    Value() = default;
...
template <>
struct UnexpectedType<Null>
{
    using error = not_null;
};

template <>
struct UnexpectedType<bool>
{
    using error = not_bool;
};

template <>
struct UnexpectedType<int64_t>
{
    using error = not_int;
};

template <>
struct UnexpectedType<double>
{
    using error = not_double;
};

template <>
struct UnexpectedType<std::string>
{
    using error = not_string;
};

template <>
struct UnexpectedType<Array>
{
    using error = not_array;
};

template <>
struct UnexpectedType<Object>
{
