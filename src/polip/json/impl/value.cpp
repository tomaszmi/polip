...
{
    static pjson::Value::const_array_iterator nop(
        const pjson::Value::variant_type& v);
...
{
    static pjson::Value::const_object_iterator nop(
        const pjson::Value::variant_type& v);
    static pjson::Value::object_iterator nop(pjson::Value::variant_type& v);
};

template <typename T>
auto constIterableElem(const pjson::Value::variant_type& v, What2Get get)
    -> decltype(ResultType<T>::nop(v))
{
...
template <typename T>
auto iterableElem(pjson::Value::variant_type& v, What2Get get)
    -> decltype(ResultType<T>::nop(v))
{
    T* got = boost::get<T>(&v);
    if (got == nullptr) {
